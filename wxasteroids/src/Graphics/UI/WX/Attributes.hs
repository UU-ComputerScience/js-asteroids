...
--upcastProp :: forall v w. (Sup w v) => Prop v -> Prop w
--upcastProp p = 
--  contramap (handleErr . (downcast :: w -> Maybe v)) p
--  where
--  handleErr = maybe (error $ "Non-existent property: " ++ propName p) id

--downcastProp :: forall v w. (Sub w v) => Prop v -> Prop w
--downcastProp p = contramap (upcast :: w -> v) p

  
--upcastProp :: forall v w. (Sup w v) => Prop v -> Prop w
--upcastProp (v :: Prop v) :: Prop w = contramap (fromJust . (downcast :: w -> Maybe v)) v

--upcastProps :: forall f v w. (Functor f, Sup w v) => f (Prop v) -> f (Prop w)
--upcastProps = fmap upcastProp
