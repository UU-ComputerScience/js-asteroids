...
-}
--------------------------------------------------------------------------------
module Graphics.UI.WX.Draw where
...
drawBitmap :: GraphicsContext_ a -> GraphicsBitmap -> Point -> Bool -> [Prop (GraphicsContext_ a)] -> IO ()
drawBitmap dc bitmap point transparent [] = do
   img <- graphicsBitmapGetNativeBitmap bitmap
   w <- getWidth img
   h <- getHeight img
   graphicsContextDrawBitmap dc bitmap (fromIntegral $ pointX point) (fromIntegral $ pointY point) w h
