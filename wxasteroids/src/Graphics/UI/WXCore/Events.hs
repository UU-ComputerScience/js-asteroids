...
-}
-----------------------------------------------------------------------------------------
module Graphics.UI.WXCore.Events where
...
eventKeyFromEvent :: KeyEvent_ a -> IO EventKey
eventKeyFromEvent event
  = do x <- keyEventGetX event
       ...
       altDown     <- return False -- keyEventAltDown event
       controlDown <- return False -- keyEventControlDown event
       shiftDown   <- return False -- keyEventShiftDown event
       metaDown    <- return False -- keyEventMetaDown event
       let modifiers = Modifiers altDown shiftDown controlDown metaDown
...
-- Should map to the javascript event key codes
wxK_LEFT = 37
wxK_RIGHT = 39
...
keyCodeToKey keyCode
  = case lookup keyCode keyCodeMap of
            Just key -> key
